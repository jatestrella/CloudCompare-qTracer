...
- **Outcrop Area Computation…** — opens `OutcropAreaDlg`. Estimates the outcrop sampling area (the P21 denominator). **Three** partitioners, picked via a combobox that swaps the parameter pane (`QStackedWidget`):
  ...
  - **Projected** (best-fit plane + 2D occupancy) — fits **one** global best-fit plane (PCA on a strided ≤200k subsample), projects all points to it, and measures the footprint by **2D grid occupancy** (`Grid cell size`, default 0.02 m; area = occupied cells × cell²). Reports the grid area, a convex-hull area (upper-bound reference, via monotone-chain on occupied-cell centers), best-fit-plane dip/dip-dir, off-plane RMS (roughness), and in-plane extent. **This is the recommended P21 denominator on a quasi-planar outcrop**: unlike Octree/Kd-tree — which sum local plane-box patches and thus accumulate a *resolution-dependent 3D rugose area* — the projected footprint is convergent and planar (matches the standard P21 sampling-window definition and the near-planar trace lengths). Added 2026-08-12 to match the method in `E:\Research\DFNExtracter\Case\6m\AREA_METHOD.md`.

  The optional patch mesh: for Octree/Kd-tree it is the per-cell plane-box polygons; for Projected it is one quad per occupied grid cell laid on the best-fit plane (skipped if >400k cells).
...
4. **Trace Clustering** — region-growing merge of close-collinear trace pieces → `Traces` group of `Merged trace N` nodes. A `Number of passes` parameter (default 1) re-runs the merge on the previous pass's output — a merged trace can reach further neighbours on a later pass; iteration stops early when the trace count no longer decreases. This works because stage 4's input reader (`traceEndpointsOf`, BFS to the shallowest 2-vertex polyline) is structure-agnostic, so the `Traces` output (whose nodes carry a `MergedPolyline`) can be fed straight back in. Mirrors stage 6's `Max passes`.
...
6. **Coplanar Plane Merging** — sequential-RANSAC consolidation of near-coplanar joint planes from stage 5 into fewer, larger plane facets → `Merged Joint Planes` group. A peer joins a seed's consensus group when its normal is within `α_max`, its centroid is within `δ_max` of the seed plane (perpendicular), **and** its centroid is within `Max centroid distance` of the seed's centroid (an in-plane proximity limit that stops coplanar-but-laterally-distant facets merging across a gap; `0` = no limit, added 2026-08-21). Iterates up to `Max passes` (default 10), stopping early when the plane count no longer decreases. Optional **Drop unmerged planes** removes facets that never merged (tracked via the `qtracer.origSources` metadata accumulated across passes). Non-destructive: the stage-5 output is kept alongside.

All stage outputs are added under one parent DB node `qTracer [<cloud name>]`.

- **Filter Traces by Length…** (post-processing, not a paper functionality) — opens `TraceLengthFilterDlg` on a selected traces group (`Traces` or `[trace pieces]`). Interactive removal of short/over-long traces with **live 3D preview**: each direct child carrying a 2-vertex polyline (found by the same BFS as P21) is one trace; a `HistogramRangeWidget` shows the length histogram with draggable min/max cursors. While open, traces outside [min,max] are hidden via `setEnabled` (both accept & reject restore the original enabled state — non-destructive). On OK, `qTracer.cpp::doTraceFilter()` builds a **new** sibling group `<name> [len min-max]` holding freshly-rebuilt bare 2-vertex polylines of the survivors, then hides the source group and selects the new one (non-destructive). This group works directly as input to **both** P21 and stage-5 Plane Fitting: `PlaneFitting` reads each child's endpoints via a structure-agnostic BFS (`traceEndpointsOf`), not a hard-coded `getChild(1)->getChild(0)`, so it accepts `Merged trace` nodes and bare filtered polylines alike (the old hard-coded access crashed on the latter — fixed 2026-08-21). Mirrors `ColorFilterDlg`'s preview/restore pattern. Added 2026-08-21.
...
- `include/OutcropArea.h` / `src/OutcropArea.cpp` — outcrop surface-area estimator with **three** partitioners (`Params::partitioner` = `Octree`, `KdTree`, or `Projected`). Shared core for the first two: `planeCubeIntersection` walks the 12 cube/AABB edges collecting sign-change intersections, CCW-sorts them on the plane, fan-triangulates for area. Octree path uses `DgmOctree::executeFunctionForAllCellsAtLevel` with per-cell PCA + planarity filter. Kd-tree path uses `ccKdTree::build(maxError, RMS, minPts, …)` + `getLeaves` — plane equation is pre-computed on each `TrueKdTree::Leaf`, leaf's AABB is derived from its point set. **Projected path** (`computeProjected`): PCA best-fit plane on a strided subsample → single pass projecting all points → 2D grid-occupancy count (packed `(int32 iu, int32 iv)` → `uint64` key in an `unordered_set`) → area = occupied×cell²; plus `convexHullArea2D` (monotone chain) on occupied-cell centers, dip/dip-dir, off-plane RMS, in-plane extents; optional footprint mesh = one quad per occupied cell on the plane. Octree/Kd-tree sum a 3D rugose area (resolution-dependent); Projected gives the convergent planar footprint (the correct P21 denominator). `Result` struct carries total area + rejection breakdown (octree/kd) or hull/dip/RMS (projected) + an optional `ccMesh`.
...
- **Pinned CC version**: this plugin builds against CloudCompare **`v2.13.1-344-g5cba96f4`** (commit `5cba96f4`, 2026-04-19). This is the known-good upstream commit for the current plugin API; record/update it whenever the plugin is confirmed to build against a newer CC. To reproduce the build, check out CC at this commit and drop the plugin into `plugins/core/Standard/qTracer/`.
